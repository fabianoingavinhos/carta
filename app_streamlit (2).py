...
def main():
    ...
    view_df["selecionado"] = view_df["idx"].apply(lambda i: int(i) in st.session_state.selected_idxs if pd.notnull(i) else False)
    ...
    # Persistência das seleções: atualiza APENAS o que está visível
    ...
    # remove apenas os que foram desmarcados no view; mantém seleções fora do filtro
